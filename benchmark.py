...
def simulate_workload(n_samples: int = 1000):
...
def run_with_stress(n_runs: int, n_samples: int, cpu_workers: int = 2) -> list[BenchmarkResult]:
...
def run_memory_stress_comparison(n_runs: int = 3) -> list[ModelResult]:
    print("\nStarting memory stress with stress-ng --vm...")
    stress = subprocess.Popen(
        ["stress-ng", "--vm", "2", "--vm-bytes", "512M", "--timeout", "120s"],
        stdout=subprocess.DEVNULL,
        stderr=subprocess.DEVNULL
    )
    print("Memory stress running! Waiting 2 seconds...\n")
    time.sleep(2)
    try:
        results = run_model_comparison(n_runs=n_runs, stressed=True)
    finally:
        stress.terminate()
        stress.wait()
...
def print_model_summary(normal: list[ModelResult], stressed: list[ModelResult], label: str = "STRESS"):
    print(f"\n{'='*55}")
    print(f"  FINAL MODEL COMPARISON SUMMARY ({label})")
    print(f"{'='*55}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--memory-stress", action="store_true", help="Compare models under RAM pressure")
    ...
    if args.memory_stress:
        ...
        print("\n=== MEMORY STRESSED ===")
        stressed = run_memory_stress_comparison(n_runs=args.runs)
        print_model_summary(normal, stressed, label="MEMORY STRESS")
        if args.report:
            from reporter import save_model_report
            save_model_report(normal, stressed, path="memory_stress_report.png")
        ...
        try:
            ...
            stress.wait()
            print("Stress-ng stopped.")
        print_model_summary(normal, stressed, label="CPU STRESS")
        if args.report:
            from reporter import save_model_report
